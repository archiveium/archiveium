...
export async function softDeleteFolderByAccountId(
	userId: string,
	accountId: string,
	trx?: Transaction<DB>
...
export async function findFolderById(folderId: string) {
	return db.selectFrom('folders').selectAll().where('id', '=', folderId).execute();
}
...
export async function insertFolder(folderInsert: FolderInsert) {
	return db.insertInto('folders').values(folderInsert).returningAll().executeTakeFirstOrThrow();
}
