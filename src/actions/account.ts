...
const editAccountSchema = z.object({
    ...
    password: z.string().trim().min(8, 'Must be of at-least 8 characters').optional().or(z.literal('')),
});

export async function ValidateExistingAccount(data: FormData, account: Account, userId: string): Promise<ValidatedAccount> {
    const validatedData = editAccountSchema.parse({
        name: data.get('name'),
        password: data.get('password')
    });
    const provider = await getProviderById(account.provider_id);
    const updatedPassword = (validatedData.password && validatedData.password != '') ? validatedData.password : account.password;
    const imapClient = await buildClient(
        account.email,
        updatedPassword,
        provider.host
    );
...
async function cacheValidatedAccount(userId: string, email: string, validatedAccount: ValidatedAccount): Promise<boolean> {
