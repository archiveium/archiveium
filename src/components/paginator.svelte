<script lang="ts">
	...
	const showingCountStart =
		paginator.currentPage - 1 === 0 ? 1 : (paginator.currentPage - 1) * paginator.resultsPerPage;
...
{#if paginator.resultCount > 0}
	<div class="card-footer d-flex align-items-center">
		<p class="m-0 text-muted">
			Showing <span>{showingCountStart}</span> to <span>{showingCountEnd}</span> of
			<span>{paginator.resultCount}</span> emails
		</p>
