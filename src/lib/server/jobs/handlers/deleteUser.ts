        ...
        if (accounts.length === 0) {
            ...
            if (accountsActive.length > 0) {
                ...
                accountsActive.forEach((account) => {
                    ...
                    promisesLimit(() => accountService.softDeleteAccountByUserId(deletedUser.id, account.id));
                });
            ...
            if (accountsPendingDeletion.length > 0) {
                // warn and do nothing
                const accountPendingDeletionIds = accountsPendingDeletion.map((account) => { return account.id; });
