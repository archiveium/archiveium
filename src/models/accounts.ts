...
export async function getAllAccountsByUserIdCount(userId: string): Promise<number> {
	const result = await sql<
		Count[]
	>`SELECT COUNT(*) FROM accounts WHERE user_id = ${userId} AND deleted = false`;
...
export async function getAccountByUserIdAndAccountId(
	userId: string,
	accountId: string
): Promise<Account> {
...
export async function getAllSyncingAccountsByUserIdCount(userId: string): Promise<number> {
	const result = await sql<
		Count[]
	>`SELECT COUNT(*) FROM accounts WHERE user_id = ${userId} AND syncing = true`;
	return result[0].count;
}
