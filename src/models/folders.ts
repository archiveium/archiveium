...
export async function insertFoldersAndAccount(
	...
	await sql.begin(async (sql) => {
		...
		const [insertedAccount] = await sql<Account[]>`INSERT INTO accounts ${sql(
			newAccount
		)} RETURNING *`;
...
export async function updateFoldersAndAccount(
	...
	await sql.begin(async (sql) => {
		...
		if (folderIdsToUpdate.length > 0) {
			await sql`UPDATE folders
            SET syncing = true
            WHERE user_id = ${userId} AND account_id = ${
				validatedExistingAccount.account.account_id
			} AND id IN ${sql(folderIdsToUpdate)}`;
		}
...
export async function getFoldersByUserIdAndAccountId(
	userId: string,
	accountId: string
): Promise<Folder[]> {
...
export async function getRemotedEmailCountForSyncedFoldersByUserId(
	userId: string
): Promise<number> {
