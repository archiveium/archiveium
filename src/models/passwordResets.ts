...
export async function createPasswordResetRequest(
	passwordResetRequest: NewPasswordRequestRequest
): Promise<void> {
...
export async function getPasswordResetRequestByTokenAndEmail(
	token: string,
	email: string
	...
	throw new RecordNotFoundException(
		`No password reset request found for token: ${token} and email: ${email}`
	);
}
