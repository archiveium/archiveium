	...
	<Card class="max-w-none">
		...
		<List tag="ul" list="none" class="max-w-none divide-y divide-gray-200 dark:divide-gray-700">
			...
			<Li class="pb-3 sm:pb-4 pt-4">
				<div class="flex items-center space-x-4">
					<div class="flex-1 min-w-0">
						<p class="text-sm font-medium text-gray-900 truncate dark:text-white">Quote Used</p>
						<p class="text-sm text-gray-500 truncate dark:text-gray-400">Maximum allowed emails {data.streamed.emails.quota}</p>
					</div>
					<div class="inline-flex items-center text-base font-semibold text-gray-900 dark:text-white">
						{#await data.streamed.emails.processed}
							Loading...
