...
describe('seed', () => {
    ...
    it('should create admin user if it does not exist', async () => {
        ...
        vi.spyOn(config, 'get')
            .mockReturnValueOnce('test@email.com')
            .mockReturnValueOnce('password');
        vi.spyOn(userService, 'findAdminUserCount').mockResolvedValueOnce(0);
        createAdminUserSpy.mockResolvedValueOnce({ insertId: BigInt(1234), numInsertedOrUpdatedRows: BigInt(1) });
    ...
    it('should throw an exception if invalid email is given', async () => {
        // arrange
        vi.spyOn(config, 'get')
            .mockReturnValueOnce('test@email')
            .mockReturnValueOnce('password');
        vi.spyOn(userService, 'findAdminUserCount').mockResolvedValueOnce(0);
