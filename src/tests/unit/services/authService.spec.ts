...
describe('authService', () => {
	describe('verifyUserPassword', () => {
		...
		it('should throw InvalidPasswordException for invalid password', async () => {
			...
			// act & assert
			expect(() => authService.verifyUserPassword(hashedPassword, password)).toThrow(InvalidPasswordException);
		});
		...
		it('should throw error code ERR_CRYPTO_TIMING_SAFE_EQUAL_LENGTH if crypto.timingSafeEqual fails', async () => {
			...
			// act & assert
			expect(() => authService.verifyUserPassword(hashedPassword, password)).toThrow('Input buffers must have the same byte length');
		});
		...
		it('should throw error code ERR_INVALID_ARG_TYPE if crypto.scryptSync fails', async () => {
			...
			expect(() => authService.verifyUserPassword(hashedPassword, password)).toThrow('The "password" argument must be of type string or an instance of ArrayBuffer, Buffer, TypedArray, or DataView. Received undefined');
		});
	});
