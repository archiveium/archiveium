...
describe('s3Service', () => {
	...
	describe('findEmailsByFolderIdAndUserId', () => {
		it('should return email with s3 data when email in given folder has source', async () => {
			...
			const folderId = '1234';
			const findEmailByFolderIdAndUserIdSpy = vi.spyOn(
				emailService,
				'findEmailByFolderIdAndUserId'
			);
			...
			expect(result).toStrictEqual([
				{ ...email, s3Data: { from: 'Not Available', subject: 'Not Available' } }
			]);
			...
			expect(s3SendSpy.mock.calls[0][0].input).toMatchObject({
				Bucket: 'emails',
				Key: `${userId}/${folderId}/${emailWithSource.id}.eml`
			});
		});
