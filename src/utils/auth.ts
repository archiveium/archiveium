...
export async function createUserSession(cookies: Cookies, user?: User): Promise<string> {
    ...
    const sessionData: SessionData = user ? { user: { id: user.id, name: user.name, email: user.email } } : {};
...
export async function saveFlashMessage(sessionId: string, flashMessage: FlashMessage): Promise<void> {
    const sessionExpiry = 60 * 60 * 24 * 7; // one week
